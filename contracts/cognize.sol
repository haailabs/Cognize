...
/**
* @title Cognize contract
* @author Nassim Dehouche
*/
import "@openzeppelin/contracts/interfaces/IERC721.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
contract Cognize is Ownable {
    ...
    */
    ...
    function isValidOptions(uint[] memory _options) internal pure returns (bool) {
        uint optionsLength = _options.length;
        if (optionsLength < 2 || optionsLength > 20) return false;

        for (uint i = 0; i < optionsLength; i++) {
            if (_options[i] >= optionsLength) return false;
        }

        return true;
    }

    function isValidProposalFee(uint _proposalFee) internal pure returns (bool) {
        return _proposalFee >= 0.01 ether && _proposalFee <= 1 ether;
    }
function submitHIP (types _HIPType, uint _numOptions, uint _numClasses, uint _duration, uint _index) 
    public 
    payable
  ...
  // Modifier to check if the HIP is closed for responses
    ...
    require(block.timestamp > HIPs[msg.sender][_id].creationDate + HIPs[msg.sender][_id].duration, "This HIP is still open for responses.");
    _;
    }

    // Modifier to check if the HIP has no responses
    modifier onlyIfNoResponses(uint _id) {
    require(HIPs[msg.sender][_id].numResponses == 0, "This HIP has already received responses.");
    _;
    }
    ...
    */
    ...
    uint fee = fees[uint(HIPs[msg.sender][_id].HIPType)];
